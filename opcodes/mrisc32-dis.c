...
/* Get opcode suffix.  */
static const char *
_get_suffix (const unsigned vec_mode, const int pack_mode,
	     const int bit_mode, const int sel_mode,
	     const int width_mode)
{
  ...
  static const char* WIDTH_SUFFIX[2][4] = {
    {".8", ".16", ".32", ".?"},	    /* Non-folding.  */
    {".8/f", ".16/f", ".32/f", ".?/f"}  /* Folding.  */
  };
  ...
  if (sel_mode >= 0 && sel_mode <= 3)
    return SEL_SUFFIX[folding][sel_mode];
  if (width_mode >= 0 && width_mode <= 3)
    return WIDTH_SUFFIX[folding][width_mode];
...
print_insn_mrisc32 (bfd_vma addr, struct disassemble_info *info)
{
  ...
  /* Default T field modes (before decoding).  */
  pack_mode = -1;
  reg_scale = 0;
  bit_mode = -1;
  sel_mode = -1;
  ...
  if (iword == 0x00000011u)
  ...
  else if ((iword & 0xfc000000u) == 0u && (iword & 0x0000007cu) != 0x7cu)
    {
      ...
      if (opcode->ft == MR32_FT_PACK)
        ...
        sel_mode = (iword >> 7) & 3;
      else if (opcode->ft == MR32_FT_WIDTH)
        width_mode = (iword >> 7) & 3;
      reg1 = _get_reg1 (iword);
      reg2 = _get_reg2 (iword);
      reg3 = _get_reg3 (iword);

      /* The canonical MOV in MRISC32 is OR rd, z, rs (non-folding)  */
      is_mov = (opcode->op == 0x11 && reg2 == 0 && bit_mode == 0 && vec_mode != 1);

      if (is_mov)
        _print_full_mnemonic (info, "mov", "");
      else
        _print_full_mnemonic (info, opcode->name,
                              _get_suffix (vec_mode, pack_mode, bit_mode, sel_mode, width_mode));
  ...
  else if ((iword & 0xfc000000u) < 0xc0000000u)
    {
      ...
      if (is_mov)
	_print_full_mnemonic (info, "mov", _get_suffix (vec_mode, -1, -1, -1, -1));
      else if (opcode->op == 0x13 && (imm_value & ~0x1fU) == 0)
        /* ASR is an alias for EBF rd, ra, #N (where N<=31)  */
	_print_full_mnemonic (info, "asr", _get_suffix (vec_mode, -1, -1, -1, -1));
      else if (opcode->op == 0x14 && (imm_value & ~0x1fU) == 0)
        /* LSR is an alias for EBFU rd, ra, #N (where N<=31)  */
	_print_full_mnemonic (info, "lsr", _get_suffix (vec_mode, -1, -1, -1, -1));
      else if (opcode->op == 0x15 && (imm_value & ~0x1fU) == 0)
        /* LSL is an alias for MKBF rd, ra, #N (where N<=31)  */
	_print_full_mnemonic (info, "lsl", _get_suffix (vec_mode, -1, -1, -1, -1));
      else
	_print_full_mnemonic (info, opcode->name, _get_suffix (vec_mode, -1, -1, -1, -1));
      arg1 = _is_scalar (opcode, vec_mode, 1) ? scalar_regs[reg1]
                                              : vector_regs[reg1];
      arg2 = _is_scalar (opcode, vec_mode, 2) ? scalar_regs[reg2]
                                              : vector_regs[reg2];
  ...
  else
    {
      ...
      _print_full_mnemonic (info, opcode->name, _get_suffix (0, -1, -1, -1, -1));
