		...
		bfd_boolean *fold)
{
  ...
  *fold = FALSE;
  ...
  /* Suffix format: [(.(b|h))|(_(1|2|3))][/f]  */

  /* Packed mode is given in the first two characters of the suffix:
     either ".b" or ".h".  */
  if (len >= 2 && suffix[0] == '.')
    {
      switch (suffix[1])
  ...
  /* BIT and SEL modes are given in the first 2-3 characters of the suffix:
       BIT: either "_pn", "_np" or "_nn"
       SEL: either "_1", "_2" or "_3"  */
  if (len >= 2 && suffix[0] == '_')
    {
      if (suffix[1] == '1')
	{
	...
	{
	  as_bad (_ ("invalid bit or sel mode"));
	  return FALSE;
	}
    }
	       ...
	       enum mr32_bitmode bit_mode, enum mr32_selmode sel_mode)
{
  ...
  /* Expand "call" to suitable instruction combinations.  */
  if (strcmp (op_str, "call") == 0)
      ...
      if (fold || pack_mode != MR32_PM_NONE)
	{
      ...
      *op2 = *op1;
      ...
      /* Use LR as a scratch register for operand 1. This is safe, since
	 LR will be overwritten by JL anyway.  */
      _clear_operand (op1);
      op1->type = MR32_OT_SREG;
      op1->reg_no = 30; /* s30 = lr  */
      ...
      if (op2->modifier == MR32_MOD_PC)
	{
	  ...
	  op2->modifier = MR32_MOD_PCLO;
	  op2->exp.X_add_number += 4;
	  _emit_instruction ("jl", op1, op2, op3, fold, pack_mode, bit_mode, sel_mode);
	}
      else if (op2->modifier == MR32_MOD_NONE)
	{
	  op2->modifier = MR32_MOD_HI;
