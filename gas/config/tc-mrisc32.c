...
_decode_suffix (char *suffix, char *suffix_end, enum mr32_packmode *pack_mode,
		enum mr32_selmode *sel_mode, bfd_boolean *fold)
{
  ...
  *fold = FALSE;
  ...
  /* SEL mode is given in the first two characters of the suffix:
     either "_1", "_2" or "_3".  */
  if (len >= 2 && suffix[0] == '_')
    {
      switch (suffix[1])
	       ...
	       enum mr32_selmode sel_mode)
{
      ...
      if (op2->exp.X_op == O_constant)
	{
	  if (op2->modifier == MR32_MOD_NONE)
	      ...
	      else
		{
		  ...
		  _emit_instruction ("or", op1, op1, op2, fold, pack_mode, sel_mode);
		}
	    }
	  else
	    {
	  ...
	  if (op2->modifier == MR32_MOD_PC)
	    {
	      ...
	      _emit_instruction ("addpchi", op1, op2, op3, fold, pack_mode, sel_mode);
	      op2->modifier = MR32_MOD_PCLO;
	      op2->exp.X_add_number += 4;
	      _emit_instruction ("add", op1, op1, op2, fold, pack_mode, sel_mode);
	    }
  ...
  if (strcmp (op_str, "call") == 0)
      ...
      if (op2->modifier == MR32_MOD_PC)
	{
	  ...
	  op2->modifier = MR32_MOD_PCLO;
	  op2->exp.X_add_number += 4;
	  _emit_instruction ("jl", op1, op2, op3, fold, pack_mode, sel_mode);
	}
      else if (op2->modifier == MR32_MOD_NONE)
	{
	  op2->modifier = MR32_MOD_HI;
...
md_assemble (char *str)
{
  ...
  if (!_decode_suffix (suffix_start, suffix_end, &pack_mode, &sel_mode, &fold))
    {
      return;
    }
  ...
  /* Some aliases expand to more than one instruction. If the instruction gets
     expanded, we're done. */
  if (_expand_alias (op_str, &operand1, &operand2, &operand3, fold, pack_mode, sel_mode))
    return;

  /* Encode and emit the instruction.  */
  _emit_instruction (op_str, &operand1, &operand2, &operand3, fold, pack_mode, sel_mode);
}
