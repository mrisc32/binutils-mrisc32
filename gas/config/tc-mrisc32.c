		...
		bfd_boolean *fold)
{
  ...
  *fold = FALSE;
  ...
  /* The pack, bit and sel modes are given by the first two or three characters
     of the suffix.  */
  if (len >= 2 && suffix[0] == '.')
    {
	...
	{
	  as_bad (_ ("invalid pack, bit or sel mode"));
	  return FALSE;
	}
    }
...
_get_imm14_reloc (struct mr32_operand_t *op)
{
  if (op->exp.X_op == O_constant)
    {
      uint32_t value = (uint32_t) op->exp.X_add_number;
      if (_can_use_imm14lo (value))
	return BFD_RELOC_MRISC32_I14;
      if (_can_use_imm14hi (value))
	return BFD_RELOC_MRISC32_IHI14;
		  ...
		  struct mr32_operand_t *op2, struct mr32_operand_t *op3)
{
  ...
  else if (str_len == 3 && (strcmp (&s_op_str[0], "ret") == 0) &&
		   ...
		   enum mr32_bitmode bit_mode, enum mr32_selmode sel_mode)
{
  ...
  if (opcode_a && ((_is_reg (op1) && _is_reg (op2) && _is_reg (op3))))
    {
      opcode = opcode_a;
	   ...
	   ((_is_reg (op1) && _is_reg (op2) && op3->type == MR32_OT_NONE)))
    {
      opcode = opcode_b;
    }
  ...
  switch (opcode->format)
	...
	iword |= ((uint32_t) op1->reg_no) << 21;
	iword |= ((uint32_t) op2->reg_no) << 16;
	iword |= ((uint32_t) op3->reg_no) << 9;
	...
	if (bit_mode != MR32_BM_NONE)
	  {
	    if (opcode->ft != MR32_FT_BIT)
	      {
		as_bad (_ ("btiwise negation mode not supported"));
		return;
	      }
	...
	switch (reloc_operand->modifier)
	  {
	  case MR32_MOD_NONE:
	    reloc_type = _get_imm14_reloc (reloc_operand);
	    break;

	  case MR32_MOD_PC:
	    reloc_type = BFD_RELOC_MRISC32_PCREL_14;
	    pc_rel = 1;
	    break;
...
static bfd_boolean
_can_use_imm21lo (const uint32_t x)
{
  return ((int32_t) x) >= -1048576 && ((int32_t) x) < 1048576;
}
	  ...
	  if (op2->modifier == MR32_MOD_NONE)
	    {
	      ...
	      if (_can_use_imm21lo (value))
		_emit_instruction ("ldli", op1, op2, op3, fold, pack_mode, bit_mode, sel_mode);
	      else if (_can_use_imm21hi (value))
		_emit_instruction ("ldhi", op1, op2, op3, fold, pack_mode, bit_mode, sel_mode);
	      else
		{
		  op2->modifier = MR32_MOD_HI;
		  _emit_instruction ("ldhi", op1, op2, op3, fold, pack_mode, bit_mode, sel_mode);
		  op2->modifier = MR32_MOD_LO;
		  _emit_instruction ("or", op1, op1, op2, fold, pack_mode, bit_mode, sel_mode);
		}
	    }
	  else
	    {
	  ...
	  if (op2->modifier == MR32_MOD_PC)
	    {
	      ...
	      _emit_instruction ("addpchi", op1, op2, op3, fold, pack_mode, bit_mode, sel_mode);
	      op2->modifier = MR32_MOD_PCLO;
	      op2->exp.X_add_number += 4;
	      _emit_instruction ("add", op1, op1, op2, fold, pack_mode, bit_mode, sel_mode);
	    }
  ...
  if (strcmp (op_str, "call") == 0 || strcmp (op_str, "tail") == 0)
    {
      ...
      /* For "call" we use LR (S30) as a scratch register. This is safe,
         since LR will be overwritten by JL anyway.
         For "tail" we use S15 as a scratch register. This is safe, since
	 S15 is neither an argument register nor a callee saved register.  */
      int scratch_reg_no = (is_call ? 30 : 15);
      ...
      if (op2->modifier == MR32_MOD_PC)
	{
	  ...
	  op2->modifier = MR32_MOD_PCLO;
	  op2->exp.X_add_number += 4;
	  _emit_instruction (jump_insn, op1, op2, op3, fold, pack_mode, bit_mode, sel_mode);
	}
      else if (op2->modifier == MR32_MOD_NONE)
	{
	  op2->modifier = MR32_MOD_HI;
...
md_assemble (char *str)
{
  ...
  if (!_decode_suffix (suffix_start, suffix_end, &pack_mode, &bit_mode, &sel_mode, &fold))
    {
      return;
    }
  ...
  /* Some aliases expand to more than one instruction. If the instruction gets
     expanded, we're done. */
  if (_expand_alias (op_str, &operand1, &operand2, &operand3, fold, pack_mode, bit_mode, sel_mode))
    return;

  /* Encode and emit the instruction.  */
  _emit_instruction (op_str, &operand1, &operand2, &operand3, fold, pack_mode, bit_mode, sel_mode);
}
