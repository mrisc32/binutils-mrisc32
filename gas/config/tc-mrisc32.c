...
static bfd_boolean
_can_use_imm21 (const uint32_t x)
{
...
{
  return (x & 0x001fffffu) == 0u;
}

static bfd_boolean
_can_use_imm21hio (const uint32_t x)
{
  return (x & 0x001fffffu) == 0x001fffffu;
}
	       ...
	       bfd_boolean fold, enum mr32_packmode pack_mode)
{
	  ...
	  if (op2->modifier == MR32_MOD_NONE)
	    {
	      ...
	      if (_can_use_imm21 (value))
		_emit_instruction ("ldli", op1, op2, op3, fold, pack_mode);
	      else if (_can_use_imm21hi (value))
		_emit_instruction ("ldhi", op1, op2, op3, fold, pack_mode);
	      else if (_can_use_imm21hio (value))
		_emit_instruction ("ldhio", op1, op2, op3, fold, pack_mode);
	      else
		{
		  op2->modifier = MR32_MOD_HI;
		  _emit_instruction ("ldhi", op1, op2, op3, fold, pack_mode);
		  op2->modifier = MR32_MOD_LO;
		  _emit_instruction ("or", op1, op1, op2, fold, pack_mode);
		}
	    }
	  else
	    {
	  ...
	  if (op2->modifier == MR32_MOD_PC)
	    {
	      ...
	      _emit_instruction ("addpchi", op1, op2, op3, fold, pack_mode);
	      op2->modifier = MR32_MOD_PCLO;
	      op2->exp.X_add_number += 4;
	      _emit_instruction ("add", op1, op1, op2, fold, pack_mode);
	    }
  ...
  if (strcmp (op_str, "call") == 0)
      ...
      if (op2->modifier == MR32_MOD_PC)
	{
	  ...
	  op2->modifier = MR32_MOD_PCLO;
	  op2->exp.X_add_number += 4;
	  _emit_instruction ("jl", op1, op2, op3, fold, pack_mode);
	}
      else if (op2->modifier == MR32_MOD_NONE)
	{
	  op2->modifier = MR32_MOD_HI;
...
md_assemble (char *str)
{
  ...
  /* Some aliases expand to more than one instruction. If the instruction gets
     expanded, we're done. */
  if (_expand_alias (op_str, &operand1, &operand2, &operand3, fold, pack_mode))
    return;

  /* Encode and emit the instruction.  */
  _emit_instruction (op_str, &operand1, &operand2, &operand3, fold, pack_mode);
}
